...
KategoriiLinks = [
    [
        "https://setec.mk/index.php?route=product/category&path=10019_10020_10023&limit=100",
        "HardDisk",
    ],
]
...
def get_opis_html(link):
    ...
    if soup.find(
        ...
        Opis_html = str(soup.find("div", id="tab-description")).replace(",", "<br>")
    return Opis_html
...
for Kategorija in range(0, len(KategoriiLinks)):
   ...
   for i in links_produkti:  # smeni vo: for i in links_produkti:
      ...
      RedenBrojIDOK.append(
         str(Reden_brojID)
      )  # ID PO KATEGORIJA SMENI NA POCETOK ZA KRATENKA RAM/HDD/GPU....
   ...
   with open(filepath6, "w", encoding="utf8", newline="") as f:
      ...
      listaelementi = list(range(0, elementi))
